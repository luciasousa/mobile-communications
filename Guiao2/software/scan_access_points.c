...
/*QUESTÕES*
a. Identifique em que linha está identificado o nome da interface wireless onde abrir a socket netlink; altere para o nome da interface wireless obtido no ponto 4 do primeiro exercício.
linha 286 - wlp3s0
...
c. Identifique que comandos Netlink são enviados (i.e., estão no formato
“NL80211_CMD_...” e em que ordem eles são enviados;

1- NL80211_CMD_TRIGGER_SCAN
2- NL80211_CMD_NEW_SCAN_RESULTS
3- NL80211_CMD_GET_SCAN
4- NL80211_CMD_SCAN_ABORTED
...
Scan is done.
NL80211_CMD_GET_SCAN sent 28 bytes to the kernel.
70:6b:b9:88:a9:0f, 5500 MHz, eduroam
58:ef:68:b7:2b:ad, 2412 MHz, LabComWiFi
28:ee:52:ae:05:0e, 2437 MHz, LabCom330
28:ee:52:ae:1d:32, 2462 MHz, LabCom331
-------------------------------------------------------

d. Identifique o comportamento de cada função de callback em função do comando enviado

error_handler -> callback for errors
finish_handler -> callback for NL_CB_FINISH
ack_handler -> callback for NL_CB_ACK
no_seq_check -> callback for NL_CB_SEQ_CHECK
...
int main() {
    int if_index = if_nametoindex("wlp3s0"); // Use this wireless interface for scanning.
